"""Main scraping pipeline for auto.ru."""
...
from .http_client import AutoruHttpClient
...
async def scrape_listings(filters: SearchFilters, client: AutoruHttpClient) -> list[ListingItem]:
...
async def scrape_and_save(filters: SearchFilters) -> int:
    """Full pipeline: scrape listings, parse cards, save to DB with source='autoru'."""
    client = AutoruHttpClient()
...
async def _process_card(client: AutoruHttpClient, item: ListingItem) -> dict | None:
