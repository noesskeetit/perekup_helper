...
class AvitoParse:
    ...
    def parse(self):
        ...
        for _index, url in enumerate(self.config.urls):
            ...
            # Follow Avito JS-level redirects (e.g. /avtomobili/toyota -> /avtomobili/toyota-ASgB...)
            if not DEBUG_MODE:
                probe_html = self.fetch_data(url=url)
                if probe_html:
                    redirect_url = self.extract_redirect_url(probe_html)
                    if redirect_url:
                        logger.info(f"Следую редиректу: {redirect_url}")
                        url = redirect_url
                ...
                for script in html_code.select('script'):
                    if script.get('type') == 'mime/invalid' and script.get('data-mfe-state') == 'true' and 'sandbox' not in script.text:
                        ...
                        if data.get('i18n', {}).get('hasMessages', {}):
                            state_data = data.get('state', {}).get('data', {})
    ...
    @staticmethod
    def extract_redirect_url(html_code) -> str | None:
        """Извлекает JS-редирект URL из mime/invalid скрипта Avito."""
        import html as html_lib
        ...
        try:
            for script in soup.select('script[type="mime/invalid"][data-mfe-state="true"]'):
                ...
                data = json.loads(html_lib.unescape(script.text))
                state_data = data.get('state', {}).get('data', {})
                if state_data.get('redirected') and state_data.get('url'):
                    return "https://www.avito.ru" + state_data['url']
        except Exception:
            pass
        return None
